...
export function start(
  ...
  const cellsToAliveNeibours: Map<number, number> = new Map<number, number>();
  cells.forEach((cell, index) => {
    let numberOfAliveNeibours = 0;
    for (let m = -1; m <= 1; m++) {
      for (let n = -1; n <= 1; n++) {
        if (!isSelf(m, n)) {
          const neibourX = cell.index.col + m;
          const neibourY = cell.index.row + n;
          if (
            neibourIsWithInBoundary(
              neibourX,
              neibourY,
              numberOfColumns,
              numberOfRows
            ...
            const neibourIndex = neibourY * numberOfColumns + neibourX;
...
function neibourIsWithInBoundary(
  neibourX: number,
  neibourY: number,
  numberOfColumns: number,
  numberOfRows: number
  ...
  return (
    neibourX > -1 &&
    neibourY > -1 &&
    neibourX < numberOfColumns &&
    neibourY < numberOfRows
  );
}
